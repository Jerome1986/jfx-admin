  ...
  <section class="services-page fill-page-layout">
    <div class="table-card fill-content-card">
      ...
      <div class="fill-content-body">
        <el-table v-loading="loading" :data="rows" row-key="id" height="100%" border empty-text="暂无商品服务">
          <el-table-column label="服务信息" min-width="260" fixed="left">
            <template #default="{ row }">
              <div class="service-cell">
                <el-image v-if="row.image" :src="row.image" fit="cover" :preview-src-list="[row.image]"
                  preview-teleported />
                <div v-else class="image-empty">暂无图片</div>
                <div>
                  <strong>{{ row.name }}</strong><small>{{ row.description || '暂无服务说明' }}</small>
                </div>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="单价" width="150" align="right"><template #default="{ row }"><strong class="price">¥{{
            formatPrice(row.unitPrice) }}</strong> /
              {{ row.unit }}</template></el-table-column>
          <el-table-column prop="sort" label="排序" width="80" align="center" />
          <el-table-column label="状态" width="100" align="center"><template #default="{ row }"><el-switch
                :model-value="row.isEnabled" inline-prompt active-text="启" inactive-text="停"
                @change="setStatus(row, Boolean($event))" /></template></el-table-column>
          <el-table-column label="更新时间" width="175"><template #default="{ row }">{{
            formatDate(row.updatedAt)
              }}</template></el-table-column>
          <el-table-column label="操作" width="130" fixed="right"><template #default="{ row }"><el-button link
                type="primary" @click="openEdit(row)">编辑</el-button><el-button link type="danger"
    ...
    <ConstructionServiceEditorDialog v-model="dialogVisible" :service-id="editingId" @saved="loadData" />
  </section>
</template>
